...
begin
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'customer_prices_company_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'sales_daily_company_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'delivery_drivers_company_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'regions_company_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'monthly_settlements_company_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'payments_company_id_fkey'
  ) then
...
create policy sales_daily_delete
on public.sales_daily
for delete
using (public.can_access_company(company_id));
...
create policy delivery_drivers_insert
on public.delivery_drivers
for insert
with check (public.can_access_company(company_id));
...
create policy monthly_settlements_delete
on public.monthly_settlements
for delete
using (public.can_access_company(company_id));
